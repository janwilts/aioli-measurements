...
class Frame:
    ...
    def remove_straights(self, min_line_length=40, max_line_gap=10, draw_length=50):
        """ Takes in the cropped images, and removes the straight lines """
        user_frame = self._frame
        user_frame = cv2.GaussianBlur(user_frame, (5, 5), 1)
        gray = cv2.cvtColor(user_frame, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(gray, 50, 150, 0)
        blurred_edges = cv2.GaussianBlur(edges, (5, 5), 0)
        lines = cv2.HoughLinesP(blurred_edges, 1, np.pi / 100, 150, min_line_length, max_line_gap)
            ...
            for line in lines:
                for x1, y1, x2, y2 in line:
                    ...
                    if dydx > 0.3 or dydx < -0.3:
                        cv2.line(user_frame, (x1, y1), (x2, y2), (0, 0, 255), 5)
