...
def main():
    ...
    for cam in cameras:
        # Loop through all the cameras in the cameras list
        if cam.cap.isOpened():
            ...
            # This code is using very exact reference points. This makes it perfect when the product-strip and reference
            # point are completely straight and the edges are really sharp. Due to slight variations in the canny
            # function due to camera movement this would read the marker to be one more pixel to the left/right compared
            # to the other vertical lines. Same story for the horizontal lines.
            ...
            # This is the return percentage from comparing both frames, final result
            # Currently unused, but the use is up to the user
...
def camera_status():
    """ Checks the status of the camera's, by snapping a test frame. Only runs once """
    global cameras
...
if __name__ == '__main__':
    ...
    # Main loop, while the cameras are OK
    while camera_status():
        main()
    ...
    # When broken, end the application
    for cam in cameras:
        cam.cap.release()
